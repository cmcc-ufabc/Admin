...
public class FaseController implements Serializable {
    ...
    @EJB 
    private FaseFacade verifica;
    
    //Inicia com a fase ativa marcada
    @PostConstruct
    public void init(){
        ativa = verifica.achaMax();
        if(!(ativa == null)){
            if(ativa.isAfinidades() == true){
                afinidades = true;
            }
            if(ativa.isFase1() == true){
                opt = "fase1";
            }
            if(ativa.isFase2_quad1() == true){
                opt = "fase21";
            }
            if(ativa.isFase2_quad2() == true){
                opt = "fase22";
            }
            if(ativa.isFase2_quad3() == true){
                opt = "fase23";
            }
        }
    }
